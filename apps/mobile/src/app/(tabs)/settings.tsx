...
export default function SettingsScreen() {
  ...
  useEffect(() => {
    api.getNotificationPrefs()
      .then((prefs) => {
        ...
        setShareRegion(prefs.shareRegion ?? true);
      })
      .catch(() => {});
  ...
  async function handleToggleShareRegion(value: boolean) {
    setShareRegion(value);
    try {
      const saved = await api.updateNotificationPrefs({ shareRegion: value });
      setShareRegion(saved.shareRegion ?? value);
    } catch {
      setShareRegion(!value);
    }
  }
  ...
  return (
    <View style={styles.container}>
      ...
      {plan === "premium" ? (
        <View style={[styles.section, { padding: 16, gap: 16 }]}>
          ...
          <View style={{ gap: 14 }}>
            <View style={spStyles.labelRow}>
              <Feather name="shield" size={15} color={BRAND.primary} />
              <Text style={profileStyles.label}>{t("settings.emergencyContact")}</Text>
            </View>
            <Text style={ecStyles.hint}>{t("settings.emergencyContactHint")}</Text>
            <View style={profileStyles.field}>
      ...
      {/* Activity Apps — deferred (iOS cannot detect third-party app opens) */}
      ...
      <View style={styles.section}>
        <SettingRow
          ...
          value={shareRegion}
          onToggle={handleToggleShareRegion}
