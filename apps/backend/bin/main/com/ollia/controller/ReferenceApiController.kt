...
class ReferenceApiController(
    ...
    fun upsertUser(@RequestBody request: UpsertUserRequest): ApiUserResponse {
        ...
        if (request.region != null) {
            user.region = PlaceResolver.normalizeRegion(request.region) ?: request.region
            dirty = true
        }
    ...
    fun setTravelMode(
        ...
        user.travelDestination = if (request.travelMode) {
            PlaceResolver.normalizeRegion(request.travelDestination) ?: request.travelDestination
        } else null
        userRepository.save(user)
        return user.toApiResponse(currentUserService.getClerkId())
    }
